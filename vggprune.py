# -*- coding: utf-8 -*-
"""
Created on Sat Oct 12 16:57:37 2019
...
parser.add_argument('--model', default='./results-vgg16-1/model_best.pth.tar', type=str, metavar='PATH',
...
with open(os.path.join(args.save, "vggprune.txt"), "w") as fp:
...
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'vggpruned.pth.tar'))
print(newmodel)
model = newmodel
acc = test(model)
acc = acc.numpy()
...
print("number of parameters: "+str(num_parameters))
with open(os.path.join(args.save, "vggpruned.txt"), "w") as fp:
    fp.write("Number of parameters: \n"+str(num_parameters)+"\n")
    fp.write("Test accuracy: \n"+str(acc)+"\n")
